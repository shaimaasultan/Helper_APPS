...
const CANVAS_SIZE = 512; // FIXED CANVAS SIZE
...
function drawCave() {
  ...
  const cellW = CANVAS_SIZE / W;
  const cellH = CANVAS_SIZE / H;
...
function drawMarker(ctx, cellX, cellY, color) {
  ...
  const px = cellX * cellW + cellW / 2;
  const py = cellY * cellH + cellH / 2;
  ...
  while (queue.length) {
    ...
    if (p.x === endCell.x && p.y === endCell.y) {
      ...
      while (cur) {
        const cellW = CANVAS_SIZE / W;
        const cellH = CANVAS_SIZE / H;
        const cx = cur.x * cellW + cellW / 2;
        const cy = cur.y * cellH + cellH / 2;
...
document.getElementById("c").addEventListener("click", (e) => {
  ...
  if (caveMap[cellY][cellX] === 1) return;
