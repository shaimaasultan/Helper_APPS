...
def archive_logs(days: int | None = None, keep_raw_files: bool | None = None):
    ...
    try:
        ...
        log_message(f"❌ Failed to close file handlers: {e}", LogLevel.ERROR)
    ...
    for patt in patterns:
        try:
            ...
            log_message(f"❌ Failed to expand pattern {patt}: {e}", LogLevel.ERROR)
        ...
        try:
            ...
            if os.path.exists(target_path):
                try:
                    ...
                    _safe_remove(path)
                    log_message(f"📎 Appended log to archive: {path} → {target_path}", LogLevel.DEBUG)
                except Exception as e:
                    log_message(f"❌ Append failed for {path} → {target_path}: {e}", LogLevel.ERROR)
                    continue
            else:
                try:
                    _safe_move(path, target_path)
                    log_message(f"📦 Archived log: {path} → {target_path}", LogLevel.DEBUG)
                except Exception as e:
                    log_message(f"❌ Move failed for {path} → {target_path}: {e}", LogLevel.ERROR)
                    continue

            touched_by_date.setdefault(date_str, []).append(target_path)

        except Exception as e:
            log_message(f"❌ Unexpected error while processing {path}: {e}", LogLevel.ERROR)
            ...
            with zipfile.ZipFile(zip_path, "a", compression=zipfile.ZIP_DEFLATED) as zf:
                for f in files:
                    try:
                        ...
                        log_message(f"❌ Failed to add to zip {f}: {e}", LogLevel.ERROR)
            ...
            if not keep_raw_files:
                for f in files:
                    try:
                        ...
                        log_message(f"❌ Failed to remove archived file {f}: {e}", LogLevel.ERROR)

            log_message(f"🗜️ Compressed {len(files)} file(s) for {date_str} → {zip_path}", LogLevel.INFO)
        except Exception as e:
            log_message(f"❌ Compression failed for {date_str}: {e}", LogLevel.ERROR)
    ...
    try:
        ...
        log_message(f"❌ Failed to reinitialize file handlers: {e}", LogLevel.ERROR)
        ...
        for f in glob.glob(os.path.join(log_dir, "*")):
            ...
            try:
                if os.path.getmtime(f) < cutoff:
                    _safe_remove(f)
                    log_message(f"🧹 Deleted old log: {f}", LogLevel.DEBUG)
            except Exception as e:
                log_message(f"❌ Failed to consider {f} for cleanup: {e}", LogLevel.ERROR)
    except Exception as e:
        log_message(f"❌ Cleanup failed: {e}", LogLevel.ERROR)
...
if __name__ == "__main__":
    ...
    log_message("Logger ready.", LogLevel.INFO)
    log_message("Logger initialized.", LogLevel.INFO)
    log_message("Disk space below 10%.", LogLevel.WARNING)
    log_message("Cache cleared for session 123.", LogLevel.DEBUG)
    log_message("SMTP server error in payment gateway", LogLevel.CRITICAL)
    log_message(cfg.LOG_DIR, LogLevel.DEBUG)
